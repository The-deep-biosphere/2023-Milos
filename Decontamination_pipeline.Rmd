...
Now we can import the data. They should be in the same directory as the script.
...
## Decontam package
Here we use only the frequency approach of the [decontam](https://www.bioconductor.org/packages/devel/bioc/vignettes/decontam/inst/doc/decontam_intro.html) package, because we do not have enough blanks for the prevalence approach. Also,  only the cardhynal dataset is used as we do not have qPCR data for the Saganaki dataset. Nevertheless, OTUs considered as contaminants were removed from both entire dataset.
``` {r decontam1}
